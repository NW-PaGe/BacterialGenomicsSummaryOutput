    ...
    for (source in unique_sources) {
      ...
      if (nrow(filtered_links) > 0) {
        filtered_links <- filtered_links %>%
         ...
         group_by(ID) %>%
         summarize(
        `StrongGenLinks (0-10 SNP)` = paste(unique(`StrongGenLinks (0-10 SNP)`)[`StrongGenLinks (0-10 SNP)` != ""], collapse = ", "),
        `InterGenLinks (11-50 SNP)` = paste(unique(`InterGenLinks (11-50 SNP)`)[`InterGenLinks (11-50 SNP)` != ""], collapse = ", ")
      ) %>%
      ungroup()
