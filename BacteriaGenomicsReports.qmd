...
Based on the SNP distances these are the strong (0-10 SNPs) and intermediate (11-50 SNPs) linkages found in these genetic clusters
...
if (nrow(summary_snp_links) > 0) {
  ...
  for (source in unique_sources) {
    ...
    if (nrow(filtered_links) > 0) {
      filtered_links <- filtered_links %>% select(-Source) %>% 
      rename(
          ID = ID1,
  ...
  cat(paste0("#### Summary of SNP Linkages for ", source, "\n\n"))

      # Create the table and print it directly
